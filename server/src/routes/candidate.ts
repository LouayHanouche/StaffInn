...
import { removeCvFileIfPresent } from '../utils/cv-files.js';
import { buildPagination } from '../utils/pagination.js';
import { normalizeSkills, sanitizeText, skillsToText } from '../utils/sanitize.js';

export const candidateRouter = Router();

const cleanupOldCvFile = async (filename: string | null | undefined): Promise<void> => {
  try {
    await removeCvFileIfPresent(filename);
  } catch (error) {
    console.error(
      JSON.stringify({
        message: 'Failed to remove stale CV file',
        filename,
...
candidateRouter.post(
  ...
  async (request, response) => {
    ...
    const previousCvPath = candidate.cvPath;
    ...
    if (previousCvPath && previousCvPath !== request.file.filename) {
      await cleanupOldCvFile(previousCvPath);
    }
...
candidateRouter.get(
  ...
  async (request, response) => {
    ...
    const cacheKey = cacheKeyFrom('candidate-search', {
      page,
      pageSize,
      skills,
      experienceMin,
